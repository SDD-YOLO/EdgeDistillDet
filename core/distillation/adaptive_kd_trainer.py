...
from ultralytics.models.yolo.detect.train import DEFAULT_CFG, DetectionTrainer
...
class AdaptiveAlphaScheduler:
    ...
    def update(self, task_loss):
        ...
        if self._prev_ema is not None:
            ...
            self.alpha = max(
                self.alpha_min,
                min(self.alpha_max, self.alpha + self.lr_alpha * (self.delta_target - d)),
            )
...
class AdaptiveKDTrainer(DetectionTrainer):
    ...
    def _on_fit_epoch_end(self, trainer):
        ...
        display_epoch = epoch + 1
        entry = next(
            (e for e in reversed(self._distill_log) if e["epoch"] == display_epoch),
            None,
        )
        ...
        for target in [
            ...
            getattr(trainer, "results_dict", None),
        ]:
    ...
    def _ensure_per_class_metrics(self, trainer):
        ...
        try:
            import io as _io2
            from contextlib import redirect_stderr as _rse2
    ...
    def save_model(self, *args, **kwargs):
        ...
        try:
            ...
            if hasattr(self, "_class_loss_backup"):
                type(self.model).loss = self._distill_model_loss
