...
def _align_and_flatten(s_cls: torch.tensor, t_cls: torch.tensor, num_classes: int) -> tuple:
    """
    ...
    if s_cls.dim() <= 2 and t_cls.dim() <= 2:
        ...
        return s_cls.reshape(-1, s_cls.shape[-1]) if s_cls.dim() >= 2 else s_cls.reshape(-1, 1), t_cls.reshape(-1, t_cls.shape[-1]) if t_cls.dim() >= 2 else t_cls.reshape(-1, 1)
    ...
    def forward(
        ...
        for s_feat, t_feat in zip(s_list, t_list):
            try:
                ...
                # 单对特征失败不影响整体
                __import__("logging").getLogger("EdgeDistillDet.Loss").warning(f"KL 散度计算跳过一对特征: {e}")
                continue
    ...
    def forward(
        ...
        for s_feat, t_feat in zip(s_list, t_list):
            try:
                ...
                __import__("logging").getLogger("EdgeDistillDet.Loss").warning(f"focal KL 计算跳过一对特征: {e}")
                continue
...
class FeatureAlignmentLoss(nn.Module):
    ...
    def __init__(self, student_channels: list[int], teacher_channels: list[int]):
        super().__init__()
        assert len(student_channels) == len(teacher_channels), f"学生/教师通道数不匹配: {student_channels} vs {teacher_channels}"
    ...
    def forward(self, student_feats: list[torch.tensor], teacher_feats: list[torch.tensor]) -> torch.tensor:
        ...
        for proj, s_f, t_f in zip(self.projectors, student_feats, teacher_feats):
            try:
                ...
                __import__("logging").getLogger("EdgeDistillDet.Loss").warning(f"特征对齐计算跳过: {e}")
                continue
...
class CompositiveDistillLoss(nn.Module):
    ...
    def __init__(
        ...
        teacher_channels: list[int] | None = None,
        temperature: float = 4.0,
        small_area_thresh: float = 0.005,
        scale_boost: float = 2.0,
    ...
    def forward(
        ...
        num_classes: int,
        temperature: float,
        student_feats: list[torch.tensor] | None = None,
        teacher_feats: list[torch.tensor] | None = None,
