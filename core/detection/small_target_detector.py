...
class DetResult:
    ...
    def summary(self) -> str:
        lines = [f'图像: {self.image_path}', f'推理耗时: {self.inference_ms:.1f} ms  |  检测总数: {len(self.detections)}', f'  tiny(<16²): {self.tiny_count}  small(<32²): {self.small_count}']
        for d in self.detections:
            x1, y1, x2, y2 = d.xyxy
            lines.append(f'  [{d.size_category:6s}] cls={d.cls} conf={d.conf:.3f} box=({x1:.0f},{y1:.0f},{x2:.0f},{y2:.0f}) area={d.area:.0f}px²')
        return '\n'.join(lines)
...
def _tile_image(img: np.ndarray, tile_size: int=640, overlap: float=0.2) -> List[Tuple[np.ndarray, int, int]]:
    """
...
def _nms_boxes(boxes: np.ndarray, scores: np.ndarray, iou_thresh: float=0.5) -> List[int]:
    """简洁 NMS，返回保留的索引列表。"""
    if len(boxes) == 0:
        return []
    ...
    while order.size > 0:
        ...
        iou = inter / (areas[i] + areas[order[1:]] - inter + 1e-08)
...
class SmallTargetDetector:
    ...
    def __init__(self, weight_path: str, base_conf: float=0.25, small_conf_factor: float=0.6, base_iou: float=0.45, device: str='cpu', tile_large: bool=False, tile_size: int=640, small_area_px: float=32 * 32):
    ...
    def _infer_single(self, img: np.ndarray, imgsz: int) -> List[SingleDetection]:
        raw = self.model.predict(img, imgsz=imgsz, conf=self.small_conf, iou=self.base_iou, device=self.device, verbose=False)
    ...
    def _infer_tiled(self, img: np.ndarray, imgsz: int) -> List[SingleDetection]:
        ...
        for tile, ox, oy in tiles:
            raw = self.model.predict(tile, imgsz=imgsz, conf=self.small_conf, iou=self.base_iou, device=self.device, verbose=False)
        ...
        for idx in keep:
            ...
            if scores[idx] < thresh:
                continue
            dets.append(SingleDetection(xyxy=(float(x1), float(y1), float(x2), float(y2)), conf=float(scores[idx]), cls=int(clses[idx]), area=float(area), size_category=_categorize_size(area)))
        return dets
    ...
    def _parse_raw(self, raw, img_shape) -> List[SingleDetection]:
        ...
        for box in raw.boxes:
            ...
            if conf < thresh:
                continue
            dets.append(SingleDetection(xyxy=(x1, y1, x2, y2), conf=conf, cls=cls, area=area, size_category=_categorize_size(area)))
        return dets

    def batch_infer_summary(self, image_paths: List[str], imgsz: int=640) -> dict:
        results = []
        for p in image_paths:
            try:
                results.append(self.infer(p, imgsz))
            except Exception as e:
                logger.info(f'  ⚠ 推理失败 {p}: {e}')
        total = len(results)
        if total == 0:
            return {}
        ...
        return {'total_images': total, 'total_dets': len(all_dets), 'tiny_dets': tiny_total, 'small_dets': small_total, 'avg_infer_ms': round(avg_ms, 2), 'avg_fps': round(1000 / avg_ms, 2) if avg_ms > 0 else 0}