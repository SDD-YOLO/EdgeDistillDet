...
const Animations = (() => {
    ...
    const CountUpAnim = {
        ...
        animateStat(statKey, value) {
            ...
            this.animate(statKey, numericValue, opts);
        },
    ...
    const Celebration = {
        ...
        trigger(options = {}) {
            ...
            // 如果已有庆祝动画，先清理再重新开始
            if (this.canvas || this.badgeEl || this.animId) {
                this._cleanup();
            }

            const overlayCreated = this._createOverlay();
            if (!overlayCreated) {
                this._showBadgeOnly(options.message || '训练完成！');
                return;
            }
        ...
        _fireConfetti() {
            ...
            this.particles = [];
            const count = prefersReducedMotion ? 30 : 140;
    ...
    return {
        ...
        LogEnhancer: {
            init(containerId = 'log-container') {
                ...
                // 防止重复渲染工具栏，确保 init() 可幂等调用
                if (this.container.parentNode.querySelector('.log-toolbar')) {
                    return;
                }
        ...
        initAll(options = {}) {
            ...
            // 可选初始化
            if (options.enableLogSearch !== false && this.LogEnhancer) {
                // 延迟初始化，确保 DOM 已就绪
                requestAnimationFrame(() => this.LogEnhancer.init('log-container'));
            }

            if (options.enableWebSocket !== false && this.RealTimeSocket) {
