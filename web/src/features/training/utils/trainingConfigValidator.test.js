...
export function runAllTests() {
  ...
  testCases.forEach((testCase, idx) => {
    const { config, expectedViolations, expectedFix, isWarning, name } = testCase;
    const { form: corrected, violations } = validateTrainingConfig(config);

    console.log(`${idx + 1}. ${name}`);
    console.log(`   预期违规数: ${expectedViolations}, 实际: ${violations.length}`);
    console.log(`   状态: ${violations.length === expectedViolations ? "✅ PASS" : "❌ FAIL"}`);
