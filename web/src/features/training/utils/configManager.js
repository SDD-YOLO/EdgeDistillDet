import {
  DEFAULT_FORM,
  inferComputeProviderFromConfig,
} from "../../../constants/trainingDefaults";
import {
...
export function mergeDistillConfigIntoForm(prev, config) {
  ...
  const inferredProvider = inferComputeProviderFromConfig(
    config,
    prev.training?.compute_provider || "local",
  );
  const incomingDistillation = omitKeys(config?.distillation, [
    "temperature",
    "schedule_type",
    "feat_layer",
    "alpha",
  ...
  const incomingTraining = omitKeys(config?.training, [
    "warm_epochs",
    "workers",
    "batch_size",
    "learning_rate",
  ...
  const incomingAdvancedDistillation = {
    ...extractAdvancedValues(
      incomingDistillation,
      BASIC_DISTILLATION_KEYS,
      DISTILLATION_ADVANCED_SECTIONS,
    ),
    ...extractAdvancedValues(
      config?.advanced?.distillation,
      BASIC_DISTILLATION_KEYS,
      DISTILLATION_ADVANCED_SECTIONS,
    ),
  };
  ...
  return {
    ...
    training: {
      ...
      ...incomingTraining,
      cloud_api: {
        ...prev.training?.cloud_api,
        ...(incomingTraining?.cloud_api || {}),
      },
      dataset_api: {
        ...prev.training?.dataset_api,
        ...(incomingTraining?.dataset_api || {}),
      },
    ...
    advanced: {
      training: {
        ...(prev.advanced?.training || {}),
        ...incomingAdvancedTraining,
      },
      distillation: {
        ...(prev.advanced?.distillation || {}),
        ...incomingAdvancedDistillation,
      },
    },
  };
}
...
export function buildConfigPayload(sourceForm) {
  ...
  const payload = {
    distillation: omitKeys(cloned.distillation, [
      "temperature",
      "schedule_type",
      "feat_layer",
      "alpha",
    ...
    training: omitKeys(cloned.training, [
      "warm_epochs",
      "workers",
      "batch_size",
      "learning_rate",
  ...
  payload.wandb = wandb;
  const exportAdvancedKeys = new Set(
    EXPORT_ADVANCED_SECTIONS.flatMap((section) =>
  ...
  payload.distillation = applyAdvancedOverrides(
    payload.distillation,
    cloned?.advanced?.distillation || {},
  );
  payload.advanced = {
