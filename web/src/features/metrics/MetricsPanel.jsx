...
function renderTrendIcon(trend) {
  ...
  if (normalized === "up") {
    return (
      <span
        className="material-icons trend-icon trend-up"
        title="上升"
        aria-label="上升"
      >
        trending_up
  ...
  if (normalized === "down") {
    return (
      <span
        className="material-icons trend-icon trend-down"
        title="下降"
        aria-label="下降"
      >
        trending_down
  ...
  return (
    <span
      className="material-icons trend-icon trend-stable"
      title="持平"
      aria-label="持平"
    >
      trending_flat
...
function MetricsPanel({ toast, active }) {
  ...
  const exportTable = () => {
    ...
    entries.forEach(([name, val]) => {
      csv += `${name},${val.best},${val.final},${val.improvement || ""},${
        val.trend || ""
      }\n`;
    });
    ...
    a.href = url;
    a.download = `training_results_${new Date()
      .toISOString()
      .slice(0, 10)}.csv`;
    a.click();
  ...
  const epochTotal = Array.isArray(chartSeriesState?.epochs)
    ? chartSeriesState.epochs.length
    : 0;
      ...
      <div className="metrics-toolbar">
        ...
        <div className="toolbar-right metrics-toolbar-right">
          <div className="chip-group">
            <Button
              variant={chartType === "loss" ? "secondary" : "outline"}
              className={`chip ${chartType === "loss" ? "active" : ""}`}
              onClick={() => setChartType("loss")}
            >
              损失
            </Button>
            <Button
              variant={chartType === "accuracy" ? "secondary" : "outline"}
              className={`chip ${chartType === "accuracy" ? "active" : ""}`}
              onClick={() => setChartType("accuracy")}
            >
              精度
            </Button>
            <Button
              variant={chartType === "lr" ? "secondary" : "outline"}
              className={`chip ${chartType === "lr" ? "active" : ""}`}
              onClick={() => setChartType("lr")}
            >
              学习率
            </Button>
            <Button
              variant={chartType === "all" ? "secondary" : "outline"}
              className={`chip ${chartType === "all" ? "active" : ""}`}
          ...
          <div className="auto-refresh-bar">
            ...
            <span className="auto-refresh-interval">
              {autoRefresh ? `${refreshLeft}s 后刷新` : "--"}
            </span>
          </div>
        </div>
      </div>
      ...
      {!hasData ? (
        ...
        <div id="metrics-content">
          <div className="metrics-overview">
            <OverviewCard
              label="最佳 mAP@50"
              value={overview["ov-map50"] || "--"}
              icon="trending_up"
            />
            <OverviewCard
              label="推理 FPS (GPU)"
              value={overview["ov-fps"] || "--"}
              icon="speed"
            />
            <OverviewCard
              label="模型参数量"
              value={overview["ov-params"] || "--"}
              icon="memory"
            />
            <OverviewCard
              label="训练总耗时"
              value={overview["ov-time"] || "--"}
              icon="timer"
            />
          </div>
              ...
              <div className="chart-header">
                ...
                <div className="chart-actions">
                  ...
                  <Button
                    ...
                    onClick={() => exportChart(lossRef, "loss-chart")}
                  >
              ...
              <div className="chart-header">
                ...
                <div className="chart-actions">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="btn-icon-sm"
                    onClick={() => exportChart(mapRef, "map-chart")}
                  >
                    <span className="material-icons">download</span>
                  </Button>
                </div>
              ...
              <div className="chart-header">
                ...
                <div className="chart-actions">
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="btn-icon-sm"
                    onClick={() => exportChart(lrRef, "lr-chart")}
                  >
                    <span className="material-icons">download</span>
                  </Button>
                </div>
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx) => {
                const chartKey = key;
                const value =
                  chartKey === "lr" ? ctx?.raw : ctx?.parsed?.y ?? ctx?.raw;
                return `${ctx.dataset.label}: ${formatMetricValueForTooltip(
                  chartKey,
                  value,
                )}`;
              },
              ...
              label: (ctx) => {
                const rawPt =
                  ctx?.dataset?.data != null && ctx.dataIndex != null
                    ...
                    : undefined;
                const value =
                  rawPt !== undefined ? rawPt : ctx?.parsed?.y ?? ctx?.raw;
                return `${ctx.dataset.label}: ${formatMetricValueForTooltip(
                  key,
                  value,
                )}`;
...
function renderAllCharts(instancesRef, refs, chartSeries, ranges) {
  ...
  const boxLoss = padToLength(
    (trainLoss.box_loss || []).slice(0, capLoss),
    capLoss,
  ).slice(startLoss);
  const clsLoss = padToLength(
    (trainLoss.cls_loss || []).slice(0, capLoss),
    capLoss,
  ).slice(startLoss);
  const dflLoss = padToLength(
    (trainLoss.dfl_loss || []).slice(0, capLoss),
    capLoss,
  ).slice(startLoss);
  ...
  const map50 = padToLength(
    (mapSeries.map50 || []).slice(0, capMap),
    capMap,
  ).slice(startMap);
  const map5095 = padToLength(
    (mapSeries.map50_95 || []).slice(0, capMap),
    capMap,
  ).slice(startMap);
  const prPrec = padToLength(
    (prEpoch.precision || []).slice(0, capMap),
    capMap,
  ).slice(startMap);
  const prRec = padToLength(
    (prEpoch.recall || []).slice(0, capMap),
    capMap,
  ).slice(startMap);
  ...
  const lr0 = padToLength((lrSeries.pg0 || []).slice(0, capLr), capLr).slice(
    startLr,
  );
  const lr1 = padToLength((lrSeries.pg1 || []).slice(0, capLr), capLr).slice(
    startLr,
  );
  const lr2 = padToLength((lrSeries.pg2 || []).slice(0, capLr), capLr).slice(
    startLr,
  );
  ...
  if (capDistill > 0) {
    ...
    dAlpha = padToLength(
      (distill.alpha || []).slice(0, capDistill),
      capDistill,
    ).slice(startDistill);
    dTemp = padToLength(
      (distill.temperature || []).slice(0, capDistill),
      capDistill,
    ).slice(startDistill);
    dKd = padToLength(
      (distill.kd_loss || []).slice(0, capDistill),
      capDistill,
    ).slice(startDistill);
  }
  ...
  renderBarChart(instancesRef, refs.classRef.current, "class", clsLabelsFull, [
    ...
    },
    {
      label: "Recall",
      data: cls.recall || [0],
      backgroundColor: "#2E7D32AA",
      borderColor: "#2E7D32",
    },
    {
      label: "Precision",
      data: cls.precision || [0],
