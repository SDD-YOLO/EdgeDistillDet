...
class ConnectionManager:
    ...
    async def connect(self, websocket):
        ...
        try:
            welcome_msg = {'type': 'welcome', 'timestamp': time.time(), 'data': {'cached_messages': len(self.message_cache), 'message': 'Connected to EdgeDistillDet Training Stream'}}
...
class TrainingCallback:
    ...
    """
    MESSAGE_TYPES = {'epoch_start': 'epoch_start', 'epoch_end': 'epoch_end', 'batch_end': 'batch_end', 'metrics': 'metrics', 'log': 'log', 'complete': 'complete', 'error': 'error', 'distill_update': 'distill_update'}
    ...
    async def on_epoch_start(self, epoch: int, total_epochs: int, **kwargs):
        """每个 epoch 开始时调用"""
        await self._send('epoch_start', {'epoch': epoch + 1, 'total_epochs': total_epochs, 'start_time': kwargs.get('start_time', time.time())})

    async def on_epoch_end(self, epoch: int, total_epochs: int, metrics: dict=None, **kwargs):
        """每个 epoch 结束时调用"""
        data = {'epoch': epoch + 1, 'total_epochs': total_epochs, 'progress': (epoch + 1) / total_epochs * 100 if total_epochs else 0}
        if metrics:
    ...
    async def on_batch_end(self, batch_idx: int, loss: float, total_batches: int, **kwargs):
        ...
        if self.batch_counter % 10 != 0:
            return
        await self._send('batch_end', {'batch': batch_idx + 1, 'total_batches': total_batches, 'loss': float(loss), 'global_step': self.batch_counter})
    ...
    async def push_distill_update(self, alpha: float, temperature: float, kd_loss: float, **extra):
        ...
        await self._send('distill_update', {'alpha': round(alpha, 6), 'temperature': round(temperature, 4), 'kd_loss': round(kd_loss, 6), **extra})
...
class TrainingManager:
    ...
    def start_training(self, training_id: str, config: dict, run_func=None, **run_kwargs):
        ...
        entry = {'id': training_id, 'config': config, 'start_time': time.time(), 'pid': None, 'process': None, 'thread': None, 'log_queue': log_queue, 'status': 'starting', 'current_epoch': 0, 'total_epochs': config.get('epochs', 0)}
    ...
    def get_status(self, training_id: str=None) -> Any:
        ...
        return {k: {'id': v['id'], 'status': v['status'], 'current_epoch': v.get('current_epoch', 0), 'total_epochs': v.get('total_epochs', 0), 'duration': round(time.time() - v['start_time'], 1) if v['start_time'] else 0} for k, v in self.active_trainings.items()}
