...
def _extract_epoch_progress(line: str):
    ...
    if bare and re.search(
        r"\b(GPU_mem|box_loss|cls_loss|dfl_loss|Instances|Size|it/s|s/it)\b",
        line,
        re.IGNORECASE,
    ):
...
def _update_status_line(line: str):
    ...
    with training_runtime.train_state_lock:
        ...
        if progress:
            (
                training_runtime.training_status["current_epoch"],
                training_runtime.training_status["total_epochs"],
            ) = progress
...
def _resolve_dataset_via_api(train_cfg: dict, cfg: dict, mode: str, checkpoint: str | None):
    ...
    data_yaml = str(result.get("data_yaml") or result.get("dataset_yaml") or result.get("dataset_path") or "").strip()
...
def _remote_polling_loop(api_cfg: dict, job_id: str):
    while True:
        ...
        try:
            ...
            status_data = _http_json_request(
                "GET",
                f"{api_cfg['status_url']}?{status_qs}",
...
def _run_training_process_safe(cmd):
    ...
    try:
        ...
        _update_status_line(f"[MEM_GUARD] 内存监控已启用 | 上限={max_memory_bytes // (1024**3)}GB | 检查间隔={memory_check_interval}行 | 连续超限={memory_overlimit_threshold}次")
...
def _check_and_enforce_memory_limit(pid: int, max_bytes: int, threshold: int) -> bool:
    ...
    if total_rss > max_bytes:
        ...
        if _mem_overlimit_count >= threshold:
            # 连续超限达到阈值 → 强制杀掉整棵进程树
            msg = f"[MEM_ALERT] ⚠️ 内存严重超标！当前 {mb_used:.1f}MB > 限制 {mb_limit:.0f}MB (连续{_mem_overlimit_count}次)，强制终止训练进程树..."
            _update_status_line(msg)
            ...
            # 未达阈值但已超限 → 警告
            _update_status_line(f"[MEM_WARN] 内存偏高: {mb_used:.0f}MB / {mb_limit:.0f}MB ({_mem_overlimit_count}/{threshold})，继续观察...")
            return True
    else:
        # 内存正常 → 重置计数器
