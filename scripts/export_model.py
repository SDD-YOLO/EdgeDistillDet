...
def main() -> int:
    ...
    if not export_path:
        export_path = _get_nested(cfg, ['advanced', 'training', 'export_path'])
        if isinstance(export_path, str):
            export_path = export_path.strip() or None
    if not export_path:
    ...
    fmt = str(payload.get('format') or _get_nested(cfg, ['advanced', 'training', 'format']) or 'onnx').lower()
    if fmt not in SUPPORTED_EXPORT_FORMATS:
        _print_line(f'ERROR: 不支持的导出格式: {fmt}')
        return 2

    keras = bool(payload.get('keras') or _get_nested(cfg, ['advanced', 'training', 'keras']))
    optimize = bool(payload.get('optimize') or _get_nested(cfg, ['advanced', 'training', 'optimize']))
    int8 = bool(payload.get('int8') or _get_nested(cfg, ['advanced', 'training', 'int8']))
    dynamic = bool(payload.get('dynamic') or _get_nested(cfg, ['advanced', 'training', 'dynamic']))
    simplify = bool(payload.get('simplify') or _get_nested(cfg, ['advanced', 'training', 'simplify']))
    opset = payload.get('opset') or _get_nested(cfg, ['advanced', 'training', 'opset'])
    workspace = payload.get('workspace') or _get_nested(cfg, ['advanced', 'training', 'workspace'])
    nms = bool(payload.get('nms') or _get_nested(cfg, ['advanced', 'training', 'nms']))
    ...
    if _supports_param(model.export, 'save_dir'):
        export_kwargs['save_dir'] = str(output_dir)
    if final_file is not None:
        if _supports_param(model.export, 'name'):
            export_kwargs['name'] = final_file.stem
        if _supports_param(model.export, 'save_name'):
            export_kwargs['save_name'] = final_file.name
