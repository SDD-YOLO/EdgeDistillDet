    ...
    try:
        ...
        if any(param.kind == inspect.Parameter.VAR_KEYWORD for param in params.values()):
            ...
            KNOWN_YOLO_EXPORT_ARGS = {
                'format', 'imgsz', 'half', 'int8', 'dynamic', 'simplify', 
                'opset', 'workspace', 'nms', 'batch', 'device', 'keras', 
                'optimize', 'verbose', 'project', 'name', 'save_dir'
            }
...
def main() -> int:
    ...
    fmt = str(
        payload.get('format')
        or _get_config_value(cfg, [['export_model', 'format'], ['advanced', 'training', 'format'], ['training', 'format']])
        or 'onnx'
    ).lower()
    ...
    keras = bool(
        payload.get('keras')
        or _get_config_value(cfg, [['export_model', 'keras'], ['advanced', 'training', 'keras'], ['training', 'keras']])
    )
    optimize = bool(
        payload.get('optimize')
        or _get_config_value(cfg, [['export_model', 'optimize'], ['advanced', 'training', 'optimize'], ['training', 'optimize']])
    )
    int8 = bool(
        payload.get('int8')
        or _get_config_value(cfg, [['export_model', 'int8'], ['advanced', 'training', 'int8'], ['training', 'int8']])
    )
    dynamic = bool(
        payload.get('dynamic')
        or _get_config_value(cfg, [['export_model', 'dynamic'], ['advanced', 'training', 'dynamic'], ['training', 'dynamic']])
    )
    simplify = bool(
        payload.get('simplify')
        or _get_config_value(cfg, [['export_model', 'simplify'], ['advanced', 'training', 'simplify'], ['training', 'simplify']])
    )
    opset = payload.get('opset') or _get_config_value(cfg, [['export_model', 'opset'], ['advanced', 'training', 'opset'], ['training', 'opset']])
    workspace = payload.get('workspace') or _get_config_value(cfg, [['export_model', 'workspace'], ['advanced', 'training', 'workspace'], ['training', 'workspace']])
    nms = bool(
        payload.get('nms')
        or _get_config_value(cfg, [['export_model', 'nms'], ['advanced', 'training', 'nms'], ['training', 'nms']])
    )
    save_dir = payload.get('save_dir') or _get_config_value(cfg, [['export_model', 'save_dir'], ['advanced', 'training', 'save_dir'], ['training', 'save_dir']])
    ...
    if weight_path.suffix == '.torchscript' and fmt != 'torchscript':
        _print_line(
            'ERROR: 当前权重文件为 TorchScript 模型，不能导出为非 TorchScript 格式。请使用 torchscript 格式，或换用原始 PyTorch 权重 (.pt/.pth)。'
        )
        return 2
    ...
    if expected_suffix and exported_paths:
        ...
        if mismatched:
            _print_line(
                f'ERROR: 导出格式为 {fmt}，但生成文件未匹配预期后缀 {expected_suffix}，当前输出: {exported_paths}'
            )
            return 2
