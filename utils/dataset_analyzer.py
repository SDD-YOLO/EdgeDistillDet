...
from utils.dataset_common import SIZE_BINS, classify_scene, find_label, parse_label, size_category
...
class DatasetAnalyzer:
    ...
    """
    SPLITS = {'train': ('train/images', 'train/labels'), 'val': ('valid/images', 'valid/labels'), 'test': ('test/images', 'test/labels')}
    ...
    def run(self) -> dict:
        report = {'dataset_root': str(self.root), 'splits': {}, 'overall': {}, 'annotation_qc': {}}
        ...
        report['overall'] = {'total_images': overall_img_count, 'total_annotations': overall_anno_count, 'size_distribution': dict(overall_size_count), 'size_ratio': {k: round(v / total_anno * 100, 2) for k, v in overall_size_count.items()}, 'scene_distribution': dict(overall_scene_count), 'avg_diversity_score': round(sum(diversity_scores) / max(len(diversity_scores), 1), 2)}
        report['annotation_qc'] = {'total_oob_boxes': qc_oob, 'total_zero_area_boxes': qc_zero, 'quality_level': '良好' if qc_oob + qc_zero == 0 else '可接受' if qc_oob + qc_zero < overall_img_count * 0.01 else '需修复'}
        self._report = report
    ...
    def _analyze_split(self, img_paths: List[str], img_dir: str, lbl_dir: str) -> dict:
        ...
        return {'total_images': len(img_paths), 'total_annotations': total_anno, 'size_distribution': dict(size_cnt), 'scene_distribution': dict(scene_cnt), 'diversity_score': div_score, 'qc': {'oob_boxes': oob_cnt, 'zero_area_boxes': zero_cnt}}
