...
SCENE_COLORS = {'sunny': '#F5A623', 'overcast': '#7B8FA1', 'dusk': '#E05C5C', 'night': '#4A90D9'}
SCENE_LABELS = {'sunny': 'Sunny / Day', 'overcast': 'Overcast / Hazy', 'dusk': 'Dusk / Golden Hr', 'night': 'Night / Low-light'}
SIZE_COLORS = {'tiny': '#E74C3C', 'small': '#E67E22', 'medium': '#3498DB', 'large': '#2ECC71'}
...
class DatasetVisualizer:
    ...
    def _collect_meta(self, limit: int) -> List[dict]:
        all_meta = []
        splits = {'train': ('train/images', 'train/labels'), 'val': ('valid/images', 'valid/labels')}
        for split, (img_rel, lbl_rel) in splits.items():
            ...
            for ip in tqdm(paths[:limit // 2], desc=f'  收集 {split}', ncols=72):
                ...
                small_boxes = [(a['cx'] * W - a['bw'] * W / 2, a['cy'] * H - a['bh'] * H / 2, a['cx'] * W + a['bw'] * W / 2, a['cy'] * H + a['bh'] * H / 2) for a in annos if a['area_px'] < SMALL_AREA_THRESH]
                all_boxes = [(a['cx'] * W - a['bw'] * W / 2, a['cy'] * H - a['bh'] * H / 2, a['cx'] * W + a['bw'] * W / 2, a['cy'] * H + a['bh'] * H / 2, a['area_px']) for a in annos]
                centers = [(a['cx'], a['cy']) for a in annos]
                if not annos:
                    continue
                all_meta.append({'path': ip, 'H': H, 'W': W, 'scene': scene, 'areas': areas, 'small_boxes': small_boxes, 'all_boxes': all_boxes, 'centers': centers, 'hue_std': hue_std, 'mean_v': mean_v})
        return all_meta
    ...
    def _plot_size_histogram(self, metas: List[dict]):
        ...
        for cat, (lo, hi) in SIZE_BINS.items():
            ...
            ax.axvspan(lo, min(hi, ax.get_xlim()[1]), alpha=0.12, color=SIZE_COLORS[cat], label=cat)
    ...
    def _plot_scene_pie(self, metas: List[dict]):
        ...
        fig.patch.set_facecolor('white')
        wedges, texts, autotexts = ax.pie(sizes, labels=labels, colors=colors, autopct='%1.1f%%', startangle=140, wedgeprops={'edgecolor': 'white', 'linewidth': 1.2})
        for t in autotexts:
            t.set_fontsize(8)
    ...
    def _plot_sample_grid(self, metas: List[dict], rows: int=3):
        ...
        for j, scene in enumerate(scenes):
            ...
            axes[0, j].set_title(SCENE_LABELS[scene], fontsize=7.5, fontweight='bold', pad=4, color=SCENE_COLORS[scene])
            for i in range(rows):
                ax = axes[i, j]
                ax.axis('off')
                if i >= len(chosen):
                ...
                for bx1, by1, bx2, by2, area in m['all_boxes']:
                    ...
                    cv2.rectangle(img, (int(max(0, bx1)), int(max(0, by1))), (int(min(W, bx2)), int(min(H, by2))), color, thick)
                if m['small_boxes']:
                ...
                ax.text(0.03, 0.04, f"{n_s} small target{('s' if n_s > 1 else '')}", transform=ax.transAxes, fontsize=5.5, color='white', va='bottom', fontweight='bold', bbox=dict(fc='#000', alpha=0.5, pad=1, lw=0, boxstyle='round,pad=0.2'))
        legend_elems = [mpatches.Patch(fc='none', ec='#00FF50', lw=1.5, label='Small target BBox'), mpatches.Patch(fc='none', ec='#aaa', lw=1.0, label='Other BBox'), mpatches.Patch(fc='white', ec='white', lw=1.5, label='Zoomed inset (3×)')]
        fig.legend(handles=legend_elems, loc='lower center', ncol=3, fontsize=7, frameon=False, bbox_to_anchor=(0.5, -0.01))
    ...
    def _plot_center_heatmap(self, metas: List[dict], bins: int=64):
        ...
        fig.patch.set_facecolor('white')
        im = ax.imshow(heatmap.T, origin='lower', aspect='equal', extent=[0, 1, 0, 1], cmap='YlOrRd', interpolation='bilinear')
