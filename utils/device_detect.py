...
def detect_best_device(preferred: Optional[str] = None) -> str:
    ...
    if preferred and preferred.strip().lower() not in ("", "auto", "none"):
        ...
        if cuda_devices:
            ...
            # 请求的 GPU 无效，回退到第一个可用 GPU
            print(f"[AutoDevice] 请求的设备 {requested_indices} 不可用，可用 GPU: {cuda_devices}，回退到 GPU {cuda_devices[0]}")
            return str(cuda_devices[0])
        else:
            # 没有 CUDA，回退 CPU
            print(f"[AutoDevice] 请求 GPU {p} 但 CUDA 不可用，回退到 CPU")
            _fix_dnnl_for_cpu()
            return "cpu"
...
def list_all_devices() -> List[Dict[str, Any]]:
    ...
    # 扫描 GPU (通过 nvidia-smi，不依赖 torch)
    try:
        ...
        pass  # nvidia-smi 不可用，忽略
    
    # 如果 nvidia-smi 失败，尝试用 torch 检测（备用）
    if len(devices) == 1:  # 只有 CPU
        try:
            import torch
            if torch.cuda.is_available():
...
def setup_device_for_trainer(train_cfg: dict) -> dict:
    ...
    # 总是通过 detect_best_device 验证/修正设备选择
    # 这会处理：空值、auto、无效 GPU 索引、CUDA 不可用等情况
    ...
    if device and device != corrected_device:
        print(f"[AutoDevice] 设备配置修正: '{device}' -> '{corrected_device}'")

    # CPU 模式：强制禁用 half/amp，避免 DNNL bf16 错误
    if corrected_device == "cpu":
        train_cfg["half"] = False
        train_cfg["amp"] = False
        # 如果配置文件里有 precision 也设为 32
        if "precision" in train_cfg:
            train_cfg["precision"] = 32
        print(f"[AutoDevice] CPU 模式: device=cpu, half=False, amp=False")
